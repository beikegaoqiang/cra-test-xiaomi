...
// ！！！把自定义的函数方法挂载到String字符串的原型上
String.prototype.formatTime = formatTime;

/*
 * queryURLParams：获取URL地址问号后面的参数信息（传参信息）（可能也包含HASH值）
 */
...
function _each(obj, callback, context = window) {
  let isLikeArray =
    _type.isArray(obj) || ("length" in obj && _type.isNumeric(obj.length));
...
window.alert = (function() {
  class Dialog {
    ...
    create() {
      let { title, confirm, handled } = this.options,
